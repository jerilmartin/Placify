...
export default function UniversityLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isLoading && isAuthenticated && user?.role !== "university" && user?.role !== "placement_officer") {
  ...
  if (!isAuthenticated || (user?.role !== "university" && user?.role !== "placement_officer")) return null;
